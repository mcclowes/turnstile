...
struct ClassifierTests {
    ...
    @Test(arguments: [
        ...
        ("cargo b --release", .compile, "cargo build --release"),
        ("cargo test --profile dev", .test, "cargo test"),
        ("swift build -c release --scratch-path .build/x", .compile, "swift build -c release"),
        ("swift build -c debug", .compile, "swift build"),
        ("swift test --configuration=release", .test, "swift test --configuration release"),
        ("xcodebuild -scheme App -configuration Release build", .compile, "xcodebuild build -configuration Release"),
